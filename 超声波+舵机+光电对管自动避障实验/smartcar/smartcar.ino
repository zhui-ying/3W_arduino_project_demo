/********************************* 深圳市航太电子有限公司 *******************************
* 实 验 名 ：超声波+舵机+光电对管自动避障实验
* 实验说明 ：在超声波+舵机自动避障实验基础上添加光电对管避障保护
*           按KEY_H键，开始运行，按下KEY_L键，停止运行
* 实验平台 ：流星5号、arduino UNO R3 
* 连接方式 ：请参考interface.h文件
* 注    意 ：因地面和轮子的差异，左右转向时需要根据实际左右转向的时间和速度
...
void CarBack()
{
	front_left_speed_duty=-(SPEED_DUTY+30);//迅速刹车，防止装上
	front_right_speed_duty=-(SPEED_DUTY+30);
}
...
char GetVoidStatus(void)
{
	...
	{
		count = 2;
		while(--count)//2ms 采集10次均要采集到前面障碍物信息，滤波
		{
...
void VoidRun()
{
	...
	if(VOID_NONE != GetVoidStatus())
	{
		...
		if(dis_direct >= dis_left && dis_direct >= dis_right)//前方距离最远
		{
			CarGo();
			return;
		}
